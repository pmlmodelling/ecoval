...
def get_units(ds, mapping):
    ...
    df_out = pd.DataFrame(
        {"key": keys, "model_unit": m_units, "observation_unit": o_units}
    )
                ...
                if len(all_locs) == 0:
                    ...
                    try:
                        if "+" in mapping[key]:
                            ...
                            df_match = ds.match_points(
                                all_locs, variables=vars, nan=nan
                            )
                            df_match = eval(
                                f"df_match.assign({key} "
                                + "= lambda x: "
                                + "+".join(["x." + x for x in mapping[key].split("+")])
                                + ")"
                            )

                        else:
                            df_match = ds.match_points(
                                all_locs, variables=mapping[key], nan=nan
                            )
                        df_match = df_match.rename(
                            columns={df_match.columns[-1]: "model"}
                        )
